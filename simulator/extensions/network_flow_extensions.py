"""This module contains the network flow extensions."""


def network_flow_step(self):
    """Method that executes the events involving the object at each time step."""
    if self.status == "active":
        # Interrupting the flow if the source or target is not available
        if self.source.status != "available" or self.target.status != "available":
            self.data_to_transfer = 0
            self.status = "interrupted"

        # Updating the flow progress according to the available bandwidth
        elif not any([bw == None for bw in self.bandwidth.values()]):
